...
├── test_artifacts.R
├── test_release.R
└── test_orchestrating.R     # NEW: Chunk function tests
```
...
- Orchestrating functions: Integration tests
...
test_that("assert_columns_exist provides helpful error messages", {
  ...
  expect_error(
    assert_columns_exist(df, c("a", "x"), source_hint = "run_ingest_standardize()"),
    regexp = "run_ingest_standardize"
  )
})
```

**Example orchestrating function test:**
```r
test_that("run_ingest_standardize returns structured list", {
  ...
  expect_true("kpro_master" %in% names(result))
  expect_true("metadata" %in% names(result))
  expect_true("artifact_id" %in% names(result))
...
- [OK] Test that orchestrating functions return proper structure
...
The `data_filters` section controls optional filtering during Chunk 1 processing:

```yaml
data_filters:
  # Remove exact duplicate detections (same Detector + DateTime + auto_id)
  # Applied in Stage 6 of run_ingest_standardize()
  # Default: true (recommended)
  remove_duplicates: true
  
  # Remove all rows where auto_id == "NoID"
  # Applied in Stage 7 of run_ingest_standardize()
...
artifacts:
  artifact_name:
    ...
    workflow: 'chunk1'  # Or '01' for legacy
...
- Architecture changes (e.g., workflow → chunk model)
...
- [ ] **Structured returns** for orchestrating functions
...
| Critical | Orchestrating function missing structured return | Fix immediately |
...
### 7.2 Standard Workflow Script Header
...
### 7.3 Standard Orchestrating Function Pattern

```r
#' Run [Chunk Name] Pipeline
#'
#' @param verbose Logical. Print progress messages. Default: FALSE.
#' @return Named list with data, metadata, and file paths.
#' @export
