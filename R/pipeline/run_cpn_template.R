...
#   Stage 5: Generate detector × night template grid
#   Stage 6: Apply recording schedule (if configured)
...
run_cpn_template <- function(kpro_master = NULL,
  ...
  detectors <- sort(unique(kpro_master$Detector))
  
  # Generate complete detector × night grid
  ...
  # Merge with actual calls
  cpn_template <- cpn_template %>%
    dplyr::left_join(calls_per_night_raw, by = c("Detector", "Night")) %>%
    dplyr::mutate(CallsPerNight = tidyr::replace_na(CallsPerNight, 0))
  ...
  # ===========================================================================
  # STAGE 6: APPLY RECORDING SCHEDULE
  # ===========================================================================
  
  if (verbose) print_stage_header("6", "Apply Recording Schedule")
  ...
  if (advanced_scheduling == "no") {
    ...
    cpn_template <- apply_schedule(
      template = cpn_template,
      uniform_start = recording_start,
      uniform_end = recording_end,
      verbose = verbose
    )
    
    if (verbose) {
      message(sprintf("  [OK] Applied uniform schedule: %s - %s",
                      recording_start, recording_end))
    }
    
  } else {
    # Advanced scheduling - add empty columns for user to fill
    cpn_template <- cpn_template %>%
      dplyr::mutate(
        StartDateTime = NA_character_,
        EndDateTime = NA_character_,
        RecordingHours = NA_real_
      )
    
    if (verbose) message("  [!] Advanced scheduling - user must fill times manually")
  }
  
  log_message("[Stage 6] Recording schedule applied")
  ...
  list(
    ...
    metadata = list(
      ...
      recording_schedule = list(
        ...
        end = study_params$processing_options$recording_end %||% "07:00:00",
