...
run_phase3_analysis_reporting <- function(phase2_result = NULL,
  ...
  if (verbose) {
    ...
    cat(sprintf("  Plots: %d\n", n_plots))
    cat(sprintf("  Report: %s\n", if(!is.null(report_path) && !is.na(report_path) && is.character(report_path)) basename(report_path) else "NOT GENERATED"))
    cat(sprintf("  Release bundle: %s\n", if(!is.null(release_bundle_path) && !is.na(release_bundle_path) && is.character(release_bundle_path)) basename(release_bundle_path) else "NOT CREATED"))
    cat("========================================\n")
    cat("\n")
    ...
    cat("  Review outputs:\n")
    cat(sprintf("  - Report: %s\n", if(!is.null(report_path) && is.character(report_path)) report_path else "NOT GENERATED"))
    cat(sprintf("  - Bundle: %s\n", if(!is.null(release_bundle_path) && is.character(release_bundle_path)) release_bundle_path else "NOT CREATED"))
    cat("========================================\n")
    cat("\n")
  }
