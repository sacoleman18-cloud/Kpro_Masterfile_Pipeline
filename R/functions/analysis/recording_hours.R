...
#      RecordingHours (Excel formula), StartTime, EndTime
...
# --------------------------------
#   =IF(EndTime<StartTime, (1-StartTime)+EndTime, EndTime-StartTime)*24
#
# =============================================================================
...
# ------------------------------------------------------------------------------
# Calculate Recording Hours (Handles Overnight)
# ------------------------------------------------------------------------------
...
#' correctly handling overnight recordings that cross midnight.
#'
#' @param start_time Character (HH:MM:SS) or NA. Recording start time.
#' @param end_time Character (HH:MM:SS) or NA. Recording end time.
#'
#' @return Numeric duration in hours, or NA if either input is NA.
#'
#' @details
#' **Overnight Handling:**
#' If end_time < start_time, assumes recording crossed midnight and
#' calculates: (24 - start_time) + end_time
#' 
#' **Examples:**
#' - 20:00:00 to 08:00:00 → 12 hours (crosses midnight)
#' - 06:00:00 to 18:00:00 → 12 hours (same day)
#' - 23:30:00 to 01:30:00 → 2 hours (crosses midnight)
#' 
#' **NA Handling:**
...
#'
#' @section CONTRACT:
#' - Accepts HH:MM:SS character inputs or NA
#' - Returns NA if either input is NA (fails gracefully)
#' - Handles overnight recordings correctly (end < start)
#' - Returns numeric hours (not negative values)
#' - Uses 24-hour clock only
#'
#' @section DOES NOT:
#' - Perform date arithmetic (only time differences)
#' - Validate clock correctness (assumes valid times)
#' - Handle time zones (assumes all times in same zone)
...
#' 
#' # Short overnight
#' calculate_recording_hours("23:30:00", "01:30:00")
#' # [1] 2
#' 
#' # NA handling
...
#'
#' @description
#' Creates a Detector × Night grid spanning the entire recording period,
...
#' 1. Creates complete grid: every detector × every night in date range
...
#'
#' @section CONTRACT:
#' - Creates row for EVERY detector × EVERY night in date range
