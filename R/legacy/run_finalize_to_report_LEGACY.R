...
run_finalize_to_report <- function(kpro_master = NULL,
  ...
  result <- list(
    finalize_cpn = list(),
    summary_stats = list(),
    plotting = list(),You are an expert R engineer tasked with refactoring a complex pipeline project. Here’s the context:

1. The legacy orchestrator, `run_finalize_to_report.R`, is now considered **legacy code**. It should **not be changed or refactored**. It is only provided for reference. 

2. The pipeline has now been split into **4 new modules**, executed in the following order:
   - finalize_cpn.R
   - summary_stats.R
   - plotting.R
   - report_release.R
   These modules **should not be numbered**, but their execution order must remain the same.

3. Your task (Phase 1) is to:
   - Take the 4 new modules and perform **DRY refactoring** internally.
   - Identify repeated or logically separable code blocks and **extract helper functions**, placing them in existing thematic R files (`utilities.R`, `plot_helpers.R`, `summary_helpers.R`) or a new module if absolutely necessary.
   - Preserve **all logging, console messaging, artifact registration, validation, and outputs** exactly as they currently occur.
   - Produce an **in-depth DRY refactor report**, including:
       - Extracted functions with suggested file placement
       - Input/output for each new module
       - Any dependencies between modules or helper functions
   - Produce **refactored code** for each module with detailed documentation and spacing preserved.
   - Produce a **refactored orchestrator function** that sequences the 4 modules and collects outputs exactly as before:
       - $finalize_cpn
       - $summary_stats
       - $plotting
       - $report_release
       ...
       - $summary
   - Ensure the modules are self-contained but still compatible with each other in execution order.

4. Your task (Phase 2) is to:
   - Review the **10 coding standards Markdown documents** for the project.
   - Identify **any standards, rules, or philosophies that are outdated** due to the split of the legacy orchestrator into 4 modules.
   - Flag **which items need updating** and propose updated language reflecting the current pipeline architecture, module structure, and orchestration approach.
   - Clearly indicate which rules remain valid and which need revisions.

**Constraints**:
- Do **not modify the legacy orchestrator** (`run_finalize_to_report.R`).
- Keep all **outputs, validation logging, artifacts, and workflow behavior identical**.
- Maintain all **documentation, formatting, and spacing** preferences in the new modules.
- Produce **human-readable reports** for both the DRY refactor (Phase 1) and the coding standards update (Phase 2).

Your output should include:
1. Phase 1 DRY refactor report
2. Refactored code for each of the 4 modules
3. Refactored orchestrator code calling the 4 modules
4. Phase 2 coding standards audit and recommended updates
    report_release = list(),
    summary = list(),
  ...
  # ---------------------------------------------------------------------------
  # Stage 13: Export Individual CSV Files (CSV-First Architecture)
  # ---------------------------------------------------------------------------
  # NEW: Export each summary statistic as an individual CSV file.
  # These CSVs are then used to build the Excel workbook, ensuring consistency.
  ...
  if (should_export("csv_detector_summary")) {
    ...
    if (verbose) message("  [SKIP] Detector summary CSV (disabled in config)")
  }
  ...
  if (should_export("csv_study_summary")) {
    ...
    if (verbose) message("  [SKIP] Study summary CSV (disabled in config)")
  }
  
  # 3. Species Summary CSV
  if (!is.null(all_summaries$species_summary) && should_export("csv_species_summary")) {
    ...
    csv_exports <- csv_exports + 1
  } else if (is.null(all_summaries$species_summary)) {
    if (verbose) message("  [SKIP] Species summary CSV (data not available)")
  } else {
    if (verbose) message("  [SKIP] Species summary CSV (disabled in config)")
  }
  
  # 4. Species Accumulation CSV (PARITY WITH OTHER SUMMARIES)
  ...
  } else if (is.null(all_summaries$species_accumulation)) {
    if (verbose) message("  [SKIP] Species accumulation CSV (data not available)")
  } else {
    if (verbose) message("  [SKIP] Species accumulation CSV (disabled in config)")
  }
  
  # 5. Hourly Summary CSV
  if (!is.null(all_summaries$hourly_summary_overall) && should_export("csv_hourly_summary_overall")) {
    ...
    csv_exports <- csv_exports + 1
  } else if (is.null(all_summaries$hourly_summary_overall)) {
    if (verbose) message("  [SKIP] Hourly summary CSV (data not available)")
  } else {
    if (verbose) message("  [SKIP] Hourly summary CSV (disabled in config)")
  }
  ...
  } else if (is.null(all_summaries$variance_components)) {
    if (verbose) message("  [SKIP] Variance components CSV (data not available)")
  } else {
    if (verbose) message("  [SKIP] Variance components CSV (disabled in config)")
  }
  ...
  # ---------------------------------------------------------------------------
  # Build Excel workbook FROM the individual CSV files (not from memory).
  ...
  # ---------------------------------------------------------------------------
  # Export formatted summary tables as PNG/HTML for inclusion in reports.
  # Tables are generated from the summaries created in Stages 7-12.
  ...
  # Check for webshot2 availability for PNG exports
  ...
  if (should_export("rds_summary_data")) {
    ...
    validation_context_summary_stats <- log_validation_event(
      ...
      details = list(
        export_format = "rds",
        file = basename(summary_rds_path),
  ...
  # ---------------------------------------------------------------------------
  # Stage 17: Finalize Summary Statistics Validation
