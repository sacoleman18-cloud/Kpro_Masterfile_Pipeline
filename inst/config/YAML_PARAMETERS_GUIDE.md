...
## SECTION 4: ARTIFACT_OUTPUTS

**NEW in v2.5 (2026-02-08)**

Controls which artifacts (outputs) are generated by the pipeline. Each parameter is a TRUE/FALSE toggle that enables or disables a specific output file. This allows you to:

- Skip outputs you don't need (faster pipeline execution)
- Generate only specific artifacts during development
- Customize outputs for different use cases
- Save disk space by disabling large artifacts

**Format:** Each parameter is `yes`/`no` (or `true`/`false`)

### artifact_outputs

**Type:** Object with multiple sub-parameters  
**Required:** NO (all defaults to `yes` if section is missing)  
**Valid Values:** Each sub-parameter is `yes`, `no`, `true`, or `false`

```yaml
artifact_outputs:
  # Summary Statistics - Compiled Excel workbook
  summary_stats_excel: yes
  
  # Summary Statistics - Individual CSV files
  ...
  # Summary Statistics - PNG/HTML exports (GT tables)
  ...
  # Summary Statistics - RDS archive
  rds_summary_data: yes
```

### Summary Statistics CSV Files

**Location:** `results/csv/summary_stats/`

Individual CSV files for each summary statistic. These are the source files used to build the Excel workbook.

| Parameter | Output File | Description |
|-----------|-------------|-------------|
| `csv_detector_summary` | `detector_summary_YYYYMMDD.csv` | Per-detector activity metrics |
| `csv_study_summary` | `study_summary_YYYYMMDD.csv` | Study-wide summary metrics |
| `csv_species_summary` | `species_summary_YYYYMMDD.csv` | Species by detector breakdown |
| `csv_species_accumulation` | `species_accumulation_YYYYMMDD.csv` | Cumulative species detections over time |
| `csv_hourly_summary_overall` | `hourly_summary_overall_YYYYMMDD.csv` | Hourly activity profile |
| `csv_variance_components` | `variance_components_YYYYMMDD.csv` | Variance decomposition |

**CSV Architecture:**
- CSV files are generated FIRST
- Excel workbook is built FROM the CSV files (not from memory)
- Each CSV is registered as a separate artifact with SHA256 hash
- Disabling CSV disables corresponding Excel sheet

**When to disable:**
- Development runs where you only need certain summaries
- When you only want Excel format (disable CSVs, keep Excel enabled)
- When disk space is limited

### Summary Statistics Excel Workbook

**Location:** `results/xlsx/summary_stats_YYYYMMDD.xlsx`

**Parameter:** `summary_stats_excel`

Multi-sheet Excel workbook compiled from the individual CSV files. Contains all enabled summary statistics as separate sheets.

**Dependencies:**
- Requires `openxlsx` R package
- Requires at least one CSV file enabled
- Built FROM CSV artifacts (ensures consistency)

**Excel Sheets:**
- "Detector Summary" (if `csv_detector_summary: yes`)
- "Study Summary" (if `csv_study_summary: yes`)
- "Species by Detector" (if `csv_species_summary: yes`)
- "Species Accumulation" (if `csv_species_accumulation: yes`)
- "Hourly Profile" (if `csv_hourly_summary_overall: yes`)

**When to disable:**
- You only need CSV format
- You're generating custom Excel reports elsewhere
- Package `openxlsx` not available

### Summary Statistics PNG Exports

**Location:** `results/figures/`

Formatted GT tables exported as high-resolution PNG images (300 DPI) suitable for presentations and publications.

| Parameter | Output File | Description |
|-----------|-------------|-------------|
| `png_detector_summary` | `detector_summary_YYYYMMDD.png` | Detector summary GT table |
| `png_study_summary` | `study_summary_YYYYMMDD.png` | Study summary GT table |
| `png_species_summary` | `species_summary_YYYYMMDD.png` | Species summary GT table |
| `png_hourly_summary_overall` | `hourly_summary_overall_YYYYMMDD.png` | Hourly activity table with peak highlighting |

**Dependencies:**
- Requires `webshot2` R package
- All PNG exports will be skipped if `webshot2` not installed

**Features:**
- 300 DPI resolution
- Publication-ready formatting
- Peak hour highlighting in hourly summary (gold background with low alpha)

**When to disable:**
- You only need HTML format
- You're generating figures separately
- Package `webshot2` not available
- Development runs where visuals aren't needed

### Summary Statistics HTML Exports

**Location:** `results/figures/`

Formatted GT tables exported as standalone HTML files that can be embedded in Quarto reports.

| Parameter | Output File | Description |
|-----------|-------------|-------------|
| `html_detector_summary` | `detector_summary_YYYYMMDD.html` | Detector summary GT table |
| `html_study_summary` | `study_summary_YYYYMMDD.html` | Study summary GT table |
| `html_species_summary` | `species_summary_YYYYMMDD.html` | Species summary GT table |
| `html_hourly_summary_overall` | `hourly_summary_overall_YYYYMMDD.html` | Hourly activity table |

**Dependencies:**
- None (gt package always available)
...
**When to disable:**
- You only need PNG format
- You're not using HTML reports
- Development runs

### Summary Statistics RDS Archive

**Location:** `results/rds/summary_data_YYYYMMDD.rds`

**Parameter:** `rds_summary_data`

R data structure containing all summary objects (data frames and lists) for programmatic access.

**Contents:**
- `detector_summary` (tibble)
- `study_summary` (named vector)
- `variance_components` (list)
- `species_summary` (tibble, if available)
- `species_accumulation` (tibble, if available)
- `hourly_summary_overall` (tibble, if available)
- `hourly_summary_by_detector` (tibble, if available)
- `metadata` (list with timestamps and counts)

**When to disable:**
- You only need CSV/Excel format
- You're not doing downstream R analysis
- Disk space is limited

### Configuration Examples

**Default Configuration (all outputs):**
```yaml
# If artifact_outputs section is missing, all outputs are enabled
# No configuration needed for default behavior
```

**Minimal Configuration (CSVs only):**
```yaml
artifact_outputs:
  # Enable only CSV exports
  csv_detector_summary: yes
  csv_study_summary: yes
  csv_species_summary: yes
  csv_species_accumulation: yes
...
**Development Configuration (Excel only):**
```yaml
artifact_outputs:
  # Enable CSVs (required for Excel)
  csv_detector_summary: yes
  csv_study_summary: yes
  csv_species_summary: yes
  csv_species_accumulation: yes
  ...
  # Keep RDS for R analysis
  rds_summary_data: yes
```

**Publication Configuration (visuals only):**
```yaml
artifact_outputs:
  # Enable CSVs (for data transparency)
  csv_detector_summary: yes
  csv_study_summary: yes
  csv_species_summary: yes
  csv_species_accumulation: yes
  ...
  # Enable Excel (for reviewers)
  summary_stats_excel: yes
  
  # Enable PNG (for manuscript)
  png_detector_summary: yes
  png_study_summary: yes
  png_species_summary: yes
  png_hourly_summary_overall: yes
  
  # Disable HTML (not needed)
  html_detector_summary: no
  html_study_summary: no
  html_species_summary: no
  html_hourly_summary_overall: no
  
  # Keep RDS (for reproducibility)
  rds_summary_data: yes
```
...
- Individual CSV files are ALWAYS generated before the Excel workbook
- Excel workbook is compiled FROM the CSV files, not from in-memory data
- This ensures consistency between CSV and Excel formats
- Disabling a CSV automatically removes its sheet from Excel

**Artifact Registration:**
- Each CSV file is registered individually with SHA256 hash
- Excel workbook is registered as a separate artifact
- Disabled artifacts are NOT registered
- Skipped artifacts are logged to console and pipeline_log.txt

**Conditional Data:**
- `species_summary` only generated if `species` column exists
- `species_accumulation` only generated if `species` and `Night` columns exist
- `hourly_summary` only generated if time data is available
- PNG exports only possible if `webshot2` package installed
- Excel export only possible if `openxlsx` package installed

**Future Enhancements:**
- Additional workflows (01-04, 06-07) will support artifact toggles in future versions
- See `docs/ARTIFACT_CONFIGURATION_ANALYSIS.md` for complete artifact inventory
...
**v2.5 (2026-02-08)**
- Added `artifact_outputs` section for configurable pipeline outputs
- Summary statistics now fully configuration-driven (Workflow 05)
- CSV-first architecture: Excel workbook built from CSV artifacts
- Individual artifact toggles for CSVs, Excel, PNG, HTML, and RDS
- All outputs integrated with artifact registry and validation logging
